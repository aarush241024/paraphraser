...
def get_synonyms(word):
    try:
        ...
        st.error(f"Error getting synonyms: {str(e)}")
        return []
...
def highlight_differences_with_synonyms(original, paraphrased, use_yellow_highlight, show_changed_words, show_longest_unchanged_words):
    try:
        ...
        for orig_sent, para_sent in zip(original_sentences, paraphrased_sentences):
            ...
            if show_longest_unchanged_words and len(highlighted_words) > 1:
        ...
        st.error(f"Error in highlighting differences: {str(e)}")
        return ""
...
def main():
    ...
    if show_tooltips and st.session_state.changed_words:
        ...
        if word_to_replace:
            ...
            if synonyms and synonyms[0]:  # Check if synonyms list is not empty and first item is not empty string
